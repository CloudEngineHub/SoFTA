...
class BaseTask():
    ...
    def _setup_robot_body_indices(self):
        ...
        if hasattr(self.config.robot, 'upper_dof_names'):
        ...
        if hasattr(self.config.robot, 'lower_dof_names'):
        ...
        if hasattr(self.config.robot, 'hips_dof_names'):
            self.hips_dof_indices = [self.dof_names.index(dof) for dof in self.config.robot.hips_dof_names]

        if hasattr(self.config.robot, 'left_ankle_dof_names'):
            self.left_ankle_dof_indices = [self.dof_names.index(dof) for dof in self.config.robot.left_ankle_dof_names]
        
        if hasattr(self.config.robot, 'right_ankle_dof_names'):
            self.right_ankle_dof_indices = [self.dof_names.index(dof) for dof in self.config.robot.right_ankle_dof_names]
        
        if hasattr(self.config.robot, 'torso_name'):
            self.torso_name = self.config.robot.torso_name
            self.torso_index = self.simulator.find_rigid_body_indice(self.torso_name)

        if hasattr(self.config.robot, 'pelvis_name'):
            self.pelvis_name = self.config.robot.pelvis_name
            self.pelvis_index = self.simulator.find_rigid_body_indice(self.pelvis_name)
