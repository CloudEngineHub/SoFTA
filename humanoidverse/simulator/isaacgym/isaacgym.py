...
class IsaacGym(BaseSimulator):
    ...
    def create_envs(self, num_envs, env_origins, base_init_state):
        ...
        # spacing = -4  # space between envs in meters
        # env_lower = gymapi.Vec3(-spacing / 2, 0.0, -spacing / 2)
        # env_upper = gymapi.Vec3(spacing / 2, spacing, spacing / 2)
        ...
        with Progress() as progress:
            ...
            for i in range(self.num_envs):
                ...
                # env_handle = self.gym.create_env(self.sim, env_lower, env_upper, 3)
    ...
    def setup_viewer(self):
        ...
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_I, "waist_pitch_up"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_K, "waist_pitch_down"
        )

        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_UP, "eef_pos_up"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_DOWN, "eef_pos_down"
        )

        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_LEFT, "eef_pos_left"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_RIGHT, "eef_pos_right"
        )

        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_PAGE_UP, "eef_pos_forward"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_PAGE_DOWN, "eef_pos_backward"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_1, "period_up"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_2, "period_down"
        )
    ...
    def push_robots(self, env_ids):
        """ Random pushes the robots. Emulates an impulse by setting a randomized base velocity. 
        """
        # if len(env_ids) == 0:
        #     return
        max_vel = 0.3
        self.robot_root_states[env_ids, 7:9] = +torch.ones_like(self.robot_root_states[env_ids, 7:9], device=str(self.device)) * 0.5
        # self.robot_root_states[env_ids, 7:9] = torch_rand_float(-max_vel, max_vel, (len(env_ids), 2), device=str(self.device))
        self.set_actor_root_state_tensor(torch.tensor(env_ids, device=str(self.device)), self.robot_root_states)
        # self.dof_pos[env_ids, 22] += 0.7
        self.set_dof_state_tensor(torch.tensor(env_ids, device=str(self.device)), self.dof_state)
        ...
        # self.apply_force_tensor[:, 28, 1] = 1000
        ...
        # self.apply_force_pos_tensor[:, 28,:] = self._rigid_body_pos[:, 28, :]
        # self.apply_rigid_body_force_at_pos_tensor(self.apply_force_tensor, self.apply_force_pos_tensor)
    ...
    def render(self, sync_frame_time=True):
        ...
        for evt in self.gym.query_viewer_action_events(self.viewer):
            if evt.action == "QUIT" and evt.value > 0:
                ...
                self.push_robots(torch.arange(self.num_envs, device=self.device))
                self.stop = True
            ...
            #     self.next_task()
                ...
                self.commands[:, 5] -= 0.1
                logger.info(f"Current Command: {self.commands[:, ]}")
            # elif evt.action == "waist_pitch_up" and evt.value > 0:
            #     self.commands[:, 7] += 0.1
            #     logger.info(f"Current Command: {self.commands[:, ]}")
            # elif evt.action == "waist_pitch_down" and evt.value > 0:
            #     self.commands[:, 7] -= 0.1
            #     logger.info(f"Current Command: {self.commands[:, ]}")
                ...
                self.apply_force_tensor[:, self.right_hand_link_index, 2] -= 1.0
                logger.info(f"Right hand force: {self.apply_force_tensor[:, self.right_hand_link_index, :]}")
            elif evt.action == "eef_pos_forward" and evt.value > 0:
                self.ee_commands[:, 1::5] += 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")
            elif evt.action == "eef_pos_backward" and evt.value > 0:
                self.ee_commands[:, 1::5] -= 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")
            elif evt.action == "eef_pos_left" and evt.value > 0:
                self.ee_commands[:, 2::5,] -= 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")
            elif evt.action == "eef_pos_right" and evt.value > 0:    
                self.ee_commands[:, 2::5] += 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")   
            elif evt.action == "eef_pos_up" and evt.value > 0:
                self.ee_commands[:, 3::5] += 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")
            elif evt.action == "eef_pos_down" and evt.value > 0:
                self.ee_commands[:, 3::5] -= 0.03
                logger.info(f"Current ee Command: {self.ee_commands[:, ]}")
            elif evt.action == "period_up" and evt.value > 0:
                self.gait_commands[:, 0] += 0.05
                logger.info(f"Current gait Command: {self.gait_commands[:, ]}")
            elif evt.action == "period_down" and evt.value > 0:
                self.gait_commands[:, 0] -= 0.05
                logger.info(f"Current gait Command: {self.gait_commands[:, ]}")
