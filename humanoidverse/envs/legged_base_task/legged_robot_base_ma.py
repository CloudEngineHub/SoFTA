...
class LeggedRobotBase(BaseTask):
    def __init__(self, config, device):
        ...
        #     self.hips_dof_id = [self.simulator._body_list.index(link) - 1 for link in self.config.robot.hips_link]
        # if "waist_link" in self.config.robot.motion:
        #     self.waist_dof_id = [self.simulator._body_list.index(link) - 1 for link in self.config.robot.waist_link]
    ...
    def _prepare_reward_function(self):
        ...
        if hasattr(self.config.rewards, "reward_weights"):
            self.reward_weights = {}
            ...
            def normalize_weights(weights):
                ...
                for key, value in weights.items():
                    if isinstance(value, float):
                        normalized[f"{key}.{key}"] = value
                        for subkey in self.reward_group_names:  
                            if subkey != key:
                                normalized[f"{key}.{subkey}"] = 0.0
                    else:
                        for subkey, subvalue in value.items():
                            normalized[f"{key}.{subkey}"] = subvalue

                return normalized

            self.reward_weights = normalize_weights(self.config.rewards.reward_weights)
    ...
    def _compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            group_name = self.reward_groups[name]
            for key in self.reward_group_names:
                self.rew_buf[key] += rew * self.reward_weights[f"{key}.{group_name}"]
    ...
    def _reward_penalty_upper_body_torques(self):
        ...
        return torch.sum(torch.square(self.torques[:, self.upper_body_action_indices]), dim=1)
    ...
    def _reward_penalty_upper_body_dof_vel(self):
        ...
        return torch.sum(torch.square(self.simulator.dof_vel[:, self.upper_body_action_indices]), dim=1)
    ...
    def _reward_penalty_upper_body_dof_acc(self):
        ...
        return torch.sum(torch.square((self.last_dof_vel[:, self.upper_body_action_indices] - \
                                       self.simulator.dof_vel[:, self.upper_body_action_indices]) / self.dt), dim=1)
    ...
    def _reward_penalty_upper_body_action_rate(self):
        ...
        return torch.sum(torch.square(self.last_actions[:, self.upper_body_action_indices] - \
                                      self.actions[:, self.upper_body_action_indices]), dim=1)
    ...
    def _reward_limits_upper_body_dof_pos(self):
        ...
        out_of_limits = -(self.simulator.dof_pos[:, self.upper_body_action_indices] - \
                          self.simulator.dof_pos_limits[self.upper_body_action_indices, 0]).clip(max=0.)
        out_of_limits += (self.simulator.dof_pos[:, self.upper_body_action_indices] - \
                          self.simulator.dof_pos_limits[self.upper_body_action_indices, 1]).clip(min=0.)
        return torch.sum(out_of_limits, dim=1)
    ...
    def _reward_limits_upper_body_dof_vel(self):
        ...
        # clip to max error = 1 rad/s per joint to avoid huge penalties
        return torch.sum((torch.abs(self.simulator.dof_vel[:, self.upper_body_action_indices]) - \
                          self.simulator.dof_vel_limits[self.upper_body_action_indices, 1]).clip(min=0., max=1.), dim=1)
    ...
    def _reward_limits_upper_body_torque(self):
        ...
        return torch.sum((torch.abs(self.torques[:, self.upper_body_action_indices]) - \
                          self.torque_limits[self.upper_body_action_indices, 1]).clip(min=0.), dim=1)
