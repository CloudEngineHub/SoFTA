...
class LeggedRobotLocomotion(LeggedRobotBase):
    def __init__(self, config, device):
        ...
        full_indices = torch.arange((self.num_upper_dofs + self.num_lower_dofs), device=self.device)
        
        # Assign based on order
        self.config.robot.body_keys = self.config.robot.get("body_keys", ["lower_body", "upper_body"])
        if self.config.robot.body_keys[0] == "upper_body":
            self.upper_body_action_indices = full_indices[:self.num_upper_dofs]
            self.lower_body_action_indices = full_indices[self.num_upper_dofs:]
        elif self.config.robot.body_keys[0] == "lower_body":
            self.lower_body_action_indices = full_indices[:self.num_lower_dofs]
            self.upper_body_action_indices = full_indices[self.num_lower_dofs:]
        else:
            raise ValueError(f"Unknown body_keys order: {self.config.robot.body_keys}")
    ...
    def _init_gait_params(self):
        # Initialize the normalized period of the swing and stance phases
        self.a_swing = torch.full((self.num_envs, 1), 0.0, device=self.device)
        self.b_swing = torch.full((self.num_envs, 1), 0.5, device=self.device)
        self.a_stance = torch.full((self.num_envs, 1), 0.5, device=self.device)
        self.b_stance = torch.full((self.num_envs, 1), 1.0, device=self.device)
        self.kappa = torch.full((self.num_envs, 1), 4.0, device=self.device)
        self.left_offset = torch.full((self.num_envs, 1), 0.0, device=self.device)
        self.right_offset = torch.full((self.num_envs, 1), 0.5, device=self.device)

        self.left_feet_height = torch.zeros((self.num_envs, 1), device=self.device)
        self.right_feet_height = torch.zeros((self.num_envs, 1), device=self.device)

        self.phase_time = torch.zeros((self.num_envs, 1), dtype=torch.float32, device=self.device, requires_grad=False)
        ...
        if hasattr(self.config.rewards, "gait_period"):
            if not self.config.rewards.gait_period:
                T = self.config.rewards.gait_period
            else:
                T = 1.0
        else:
            T = 1.0
        self.T = torch.full((self.num_envs, 1), T, device=self.device)

        if hasattr(self.config.obs, "use_phase") and self.config.obs.use_phase:
            self.phi_offset = torch.rand((self.num_envs, 1), device=self.device) * self.T
        else:
            self.phi_offset = torch.zeros((self.num_envs, 1), device=self.device)

        # Arm joint positions (shared across envs, so no shape change)
        self.swing_arm_joint_pos = torch.tensor([-1.04, 0.0, 0.0, 1.57,
                                                0.0, 0.0, 0.0], device=self.device, dtype=torch.float32, requires_grad=False)
        self.stance_arm_joint_pos = torch.tensor([0.757, 0.0, 0.0, 1.57,
                                                0.0, 0.0, 0.0], device=self.device, dtype=torch.float32, requires_grad=False)

    def _calc_phase_time(self):
        # Calculate the phase time
        phase_time = (self.episode_length_buf.unsqueeze(1) * self.dt + self.phi_offset) % self.T / self.T
        phase_time *= self.commands[:, 4].unsqueeze(1) # only apply when locomotion
        return phase_time
    ...
    def _reward_penalty_waist_dofs(self):
        ...
        waist_dofs_error = torch.sum(torch.square(waist_pos - waist_pos*0), dim=1)
    ...
    def _reward_penalty_upper_body_dofs_freeze(self):
        ...
        assert self.config.robot.has_upper_body_dofwaist
    ...
    def _reward_penalty_hip_pos(self):
        ...
        hips_roll_yaw_indices = self.hips_dof_indices[1:3] + self.hips_dof_indices[4:6]
    ...
    def _get_obs_sin_phase(self):
        return torch.sin(2 * np.pi * self.phase_time)
    
    def _get_obs_cos_phase(self):
        return torch.cos(2 * np.pi * self.phase_time)